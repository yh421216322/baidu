...
namespace YourGameNamespace.Research
{
    ...
    public class ResearchSystem : AbstractSystem, IResearchSystem, IController // 实现IController以便获取System
    {
        ...
        public bool StartResearch(string techId)
        {
            ...
            {
                Debug.LogWarning($"无法开始研究技术 {techId}。因为 {mCurrentResearch.Name.Value} 的研究已在进行中。"); // Use .Value for BindableProperty
                return false;
            }
            ...
            {
                Debug.LogWarning($"技术 {techToResearch.Name.Value} (ID: {techId}) 当前不可研究。其状态为：{techToResearch.Status.Value}");
                return false;
            }

            if (!mResourceModel.ConsumeResource(GameResourceType.ResearchPoints, techToResearch.ResearchPointCost))
            {
                Debug.LogWarning($"研究点不足，无法开始研究 {techToResearch.Name.Value}。需要研究点：{techToResearch.ResearchPointCost}，当前可用：{mResourceModel.GetAmount(GameResourceType.ResearchPoints)}");
                return false;
            }
            ...
            Debug.Log($"开始研究：{mCurrentResearch.Name.Value}。所需研究点/时间：{mCurrentResearch.ResearchPointCost}。");
            return true;
        }
        ...
        private void CompleteResearch(string techId)
        {
            ...
            Debug.LogWarning($"研究完成：{completedTech.Name.Value}！");
            mResearchModel.UpdateTechnologyStatus(techId, ResearchStatus.Completed);

            Debug.Log($"正在应用技术 {completedTech.Name.Value} 的效果：");
            ...
            foreach (var effect in completedTech.Effects)
            {
                ...
                switch (effect.EffectType)
                {
                    ...
                    case TechnologyEffectType.UnlockWorkstation:
                        Debug.Log($"工作站类型 {(YourGameNamespace.Workstations.WorkstationType)effect.Value} 现在因完成研究 {completedTech.Name.Value} 而被视为已解锁！实际解锁逻辑应由WorkstationSystem处理或通过事件通知。");
                        break;
            
                    default:
                        Debug.LogWarning($"技术 {completedTech.Name.Value} 的效果类型 {effect.EffectType} 的应用逻辑尚未实现。");
                        break;
                }
            }
        ...
        public string GetTechnologyNameById(string techId)
        {
            ...
            return mResearchModel?.GetTechnology(techId)?.Name.Value;
        }
    }
}
