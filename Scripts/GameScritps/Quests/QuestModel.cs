...
namespace YourGameNamespace.Quests
{
    ...
    public class QuestModel : AbstractModel
    {
        ...
        public void UpdateQuestStatus(string questId, QuestStatus status)
        {
            ...
            {
                quest.Status = status; // 更新任务状态
                if (status == QuestStatus.Active && !ActiveQuests.Contains(quest)) // 如果任务变为活动且不在活动列表中
                {
                    ActiveQuests.Add(quest); // 添加到活动列表
                }
                else if (status != QuestStatus.Active && ActiveQuests.Contains(quest)) // 如果任务不再活动且仍在活动列表中
                {
