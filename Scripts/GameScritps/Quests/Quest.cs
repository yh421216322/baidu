...
namespace YourGameNamespace.Quests
{
    ...
    {
        NotStarted, // 未开始：任务已创建但未满足前置条件，或未被玩家查看
        Locked,     // 锁定：前置条件未满足，任务不可用
        Available,  // 可用：前置条件已满足，任务可以开始（或已自动开始，等待玩家查看）
        Active,     // 活动：任务正在进行中
        Success,    // 成功：所有目标均已达成
        Failure      // 失败：(可选状态) 如果任务存在失败条件且已被触发
    }
    ...
    public class Quest
    {
        ...
        public BindableProperty<QuestStatus> Status { get; private set; } // 任务的当前状态
                     ...
                     List<QuestReward> rewards = null)
        {
            ...
            this.Status = new BindableProperty<QuestStatus>(QuestStatus.Locked); // 新创建的任务默认为锁定状态
