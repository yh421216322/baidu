...
public class GameInitializer : MonoBehaviour
    {
        void Awake() // Unity生命周期方法，在对象实例化后立即调用
        {
            ...
            var workstationSystem = GameArchitecture.Interface.GetSystem<WorkstationSystem>(); // 获取工作站系统
            var survivorModel = GameArchitecture.Interface.GetModel<SurvivorModel>();         // 获取幸存者模型
            var workstationModel = GameArchitecture.Interface.GetModel<WorkstationModel>();   // 获取工作站模型
            var combatSystem = GameArchitecture.Interface.GetSystem<CombatSystem>();         // 获取战斗系统
            var survivorManager = GameArchitecture.Interface.GetSystem<SurvivorManagerSystem>(); // 获取幸存者管理器系统
